...
\begin{homeworkProblem}
    \begin{enumerate}
        ...
        \item Figures are shown in \texttt{src/output/p05c\_lwr\_tau{(number)}.png}. \( \tau = 0.05 \) achieves the lowest MSE on the
        \texttt{valid} split, and the MSE on the \texttt{test} split is \texttt{0.016990143386866628} with \( \tau = 0.05 \).
    \end{enumerate}
\end{homeworkProblem}
