...
\begin{homeworkProblem}
    ...
    \item Firstly, simplify the formula of \( l \):
        ...
        \]
        let \( \frac{\partial}{\partial \Sigma} l = 0 \), we have
        \[
            \begin{aligned}
                \frac{\partial}{\partial \Sigma} l &= -\frac{m}{2} \Sigma^{-1} + \frac{1}{2} \sum_{i=1}^m \Sigma^{-1} (x^{(i)} - \mu_{y^{(i)}}) {(x^{(i)} - \mu_{y^{(i)}})}^T \Sigma^{-1} = 0 \\
